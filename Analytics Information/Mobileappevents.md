...
- **Staff / moderation / achievements:** Admin reports opened/denied/expand/snapshot loaded/ban-unsuspend finalized; profile staff CTA; achievements screen started + empty-definitions shown once; user profile modal opened + report submitted; **badge earned** full-screen celebration (flip to check + haptics) when a new `users/{uid}/notifications` doc has `type: badge_earned` (after the first snapshot batch).
...
- **Manual backfill (admin):** Callable **`adminRunDerivedMetricsForUtcRange`** with **`start_date_utc`** / **`end_date_utc`** (inclusive, max 62 days) — same writers; skips days with no **`daily_metrics`** doc. **Admin → Mobile analytics** runs it from the date range controls and shows **human-readable** rates, first-touch counts (`time_to_first_activity` on `derived_metrics`), and funnel **drop-off** rows; split **JSON downloads** for Phase 2–3 raw events, Phase 4 summaries, Phase 5 `derived_metrics`, or a full bundle. **UI:** the derived-metrics list sits in a **bounded scroll panel** (not overlapping cards); **raw JSON** for each UTC day, yesterday/today snapshot cards, job responses, and range-level **`daily_metrics_by_date` / `derived_metrics_by_date`** dumps are **hidden until you click to expand** (lazy-mounted `<pre>`, no Radix `ScrollArea` around those blocks) so large payloads are never painted over the metric grid. Other scrollable panels on the same screen use plain **`overflow-auto`** containers.
...
### How the feature works (Phase 6 — badges / rewards, shared Firestore)

- **Definitions:** Staff edit **`badge_definitions/{badgeId}`** in **Digital Curriculum → Admin → Badges** and **Mortar Next.js → Admin → Badges** (same `AdminBadgesPanel.tsx` pattern in each app). The form’s **Where this badge applies** control toggles **Digital Curriculum**, **Expansion app**, or **Both** (stored as **`platform`**). Each definition includes optional **`image_url`**, that **`platform`**, and a Phase 6 **`rule`** (`metric_key`, `operator`, `threshold`, `timeframe: all_time`) against **`user_analytics_summary/{uid}.counts`**. Choosing **Expansion app** (or **Both**) switches the metric dropdown to Expansion rollup keys such as **`total_posts_created`**, **`total_messages_sent`**, **`total_group_thread_messages_sent`** (from **`group_thread_comment_submitted`**; still increments **`total_comments`**), **`total_matching_runs_started`** / **`total_matching_runs_succeeded`** (from **`matching_callable_started`** / **`matching_callable_succeeded`**), **`total_groups_joined`**, etc. (see `functions/src/analytics/summary/expansionMobileEventRollup.ts`). Digital Curriculum presets use web/curriculum analytics keys (lessons, quizzes, shop, …).
- **Awards (server):** **`onUserAnalyticsSummaryWritten`** runs **`evaluateAnalyticsBadgesForUser`** (`functions/src/analytics/badges/analyticsBadgeEvaluator.ts`), upserts **`user_badges/{uid}/awarded/{badgeId}`**, **`badge_progress/{uid}`**, appends one-time ids to **`users/{uid}.badges.earned`**, and writes a **`users/{uid}/notifications`** doc with **`type: badge_earned`**, **`badgeId`**, **`title`**, **`read: false`**.

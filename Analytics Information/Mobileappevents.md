...
- **Definitions:** Staff edit **`badge_definitions/{badgeId}`** in **Digital Curriculum → Admin → Badges** (`AdminBadgesPanel.tsx`). Each definition includes optional **`image_url`**, **`platform`** (`digital_curriculum` | `expansion_mobile` | `both`), and a Phase 6 **`rule`** (`metric_key`, `operator`, `threshold`, `timeframe: all_time`) against **`user_analytics_summary/{uid}.counts`**. Expansion-only presets use rollup keys such as **`total_posts_created`**, **`total_messages_sent`**, **`total_groups_joined`**, etc. (see `functions/src/analytics/summary/expansionMobileEventRollup.ts`).

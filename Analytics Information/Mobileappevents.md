...
- **Admin UI:** **Digital Curriculum → Admin Panel → Mobile analytics** tab (`Digital Curriculum/src/app/pages/Admin.tsx`) and **Mortar Next.js → Admin → Mobile analytics** (`web/src/app/(dashboard)/admin/page.tsx`) both load summaries via **`getAdminMobileAnalyticsDashboard`** and export raw rows via paginated **`queryAdminExpansionAnalyticsEvents`** (merged JSON download).

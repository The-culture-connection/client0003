...
When a learner completes a course, `createSkillCertificatesForCompletedCourse` reads `modules[].skills` and `modules[].skillCertificates` and creates `users/{uid}/certificates` docs that include the skill, recipient name, and uploaded `certificatePdfUrl` when provided. In **Data Room**, download behavior is now template-aware: if a PDF URL exists it opens that public file; otherwise the app generates a styled certificate template in-browser and auto-fills the learner name + skill before print/download. **Add to LinkedIn** copies the public certificate URL when available and shows: “To add a certificate to your LinkedIn profile, navigate to your profile page, click Add profile section, select Recommended, and then click Add licenses & certifications.”

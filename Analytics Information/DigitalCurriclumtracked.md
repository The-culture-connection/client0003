...
### How the feature works (Digital Curriculum alumni — completed course access)

When a learner is admitted via **`admitUserToAlumni`**, **`Digital Curriculum Students`** is removed from **`users/{uid}.roles`** and **`Digital Curriculum Alumni`** is added. Course catalog loading (**`getCoursesForLearner`** in `courses.ts`, used by **`Curriculum.tsx`** and **`Dashboard.tsx`**) unions (1) courses assigned by current role or user id with (2) any course id present in **`courseProgress`** for that user. Alumni therefore keep seeing and opening modules they started or completed as students even when those courses are only assigned to the student role. Lesson/course detail pages were already readable for signed-in users; this fix restores the curriculum list and dashboard continue-learning paths after graduation.

After a learner completes onboarding in `Onboarding.tsx`, the app writes `users/{uid}.onboarding_status = "complete"` and shows the in-page completion card. Cloud Function **`onUserOnboardingWelcomeEmail`** sends the Brevo template **`masters_onboarding_welcome`** once per user (copy in `docs/brevo-templates/html/masters_onboarding_welcome.html`; dedupe field `brevo_email_onboarding_welcome_sent_at`). Params include portal URL, learner email, first lesson title from course mapping, and start link. Respects **`email_pref_course_nudges`**.

Instead of sending the learner straight to the dashboard, the completion flow now routes them to **`/mortar-info`** (Digital Curriculum web) so they see a marketing-style landing aligned with [MORTAR Masters Online on Thinkific](https://mortarmastersonline.thinkific.com/courses/mortar-masters-online): full-width **banner** (`public/mortar-info/banner.png`), hero copy (**Build your business. Own your legacy.** + since-2014 subtitle), **Dashboard** / **Get started now** CTAs, **sponsor** strip image, embedded **YouTube** preview (`ScN2yhwYoqw`), **“what it takes”** challenge block with dreamers/doers image, and a three-module **curriculum outline** (First Verse / The Core / Really Real with lesson titles). Static assets live under `Digital Curriculum/public/mortar-info/`. The page is a normal authenticated route (under `OnboardingGate`) and tracks screen session label **`mortar_info`** via `useScreenAnalytics`.
...
- **Merge lessons:** Between two lessons in a module, **Merge with next lesson** combines the current lesson with the one below it. The **first lesson’s title** is kept. **Slides** are appended in order (all of lesson 1, then all of lesson 2). **Surveys** from the second lesson have their “after slide” index shifted by the first lesson’s slide count; end-of-lesson surveys stay at the end. **Quiz questions** from the second lesson append to the first when both have quizzes. For saved courses (both lessons have Firestore `lessonId`s), merge writes combined `lesson_content` to the first lesson, updates `lessonSurveys` / `lessonQuizzes` on the course, deletes the second lesson document, then removes the second lesson from the builder list — **Save course** afterward updates `curriculumMapping` and `modules` metadata.

### How the feature works (lesson survey — optional OpenAI analysis)

In **Course Builder**, each lesson supports **multiple surveys** placed anywhere in the slide flow. Click **Add survey** under **Lesson surveys**, choose **After slide N** or **End of lesson**, then configure name, questions, optional PDF export, and optional AI analysis per survey. Surveys are stored on `courses/{courseId}/lessonSurveys/{lessonId}` as `{ checkpoints: LessonSurveyCheckpoint[] }` (legacy single-survey docs are migrated on read). Learners see each survey when they tap **Next** after the configured slide; end-of-lesson surveys appear after the last slide (before the quiz, if any). Progress tracks each checkpoint under `courseProgress.surveySubmitted` / `surveyAnswers` keys `{lessonId}::{surveyId}`. The **`analyzeLessonSurvey`** callable accepts optional `survey_id` to analyze a specific checkpoint.
...
When a learner completes a course, `createSkillCertificatesForCompletedCourse` reads `modules[].skills` and `modules[].skillCertificates` and creates `users/{uid}/certificates` docs that include the skill, recipient name, and uploaded `certificatePdfUrl` when provided. It also merges the course-earned skills into `users/{uid}.confident_skills` using `arrayUnion(...)` so the learner’s profile reflects newly earned skills. In **Data Room**, download / LinkedIn actions are template-aware and self-healing: if a public PDF URL exists it opens/copies that link; if missing, the app generates a new PDF from the certificate template (tries `Certificate Template.pdf`, then fallback `Certificate template.pdf`), overlays learner name + skill, uploads to Storage under `users/{uid}/dataroom/certificates/...`, and returns/copies the public URL even if certificate-doc backfill update fails in that environment. **Add to LinkedIn** then shows: “To add a certificate to your LinkedIn profile, navigate to your profile page, click Add profile section, select Recommended, and then click Add licenses & certifications.”

Module-level badges are now configurable in the course creation flow with a create-only inline flow (Wizard + Builder): admins create a brand-new badge directly from each module card using “Create and attach badge,” with name/description and optional image assignment (direct image URL or file upload to Storage). The existing-badge picker is removed from module setup so badge creation stays fully inline for module completion use cases. Inline-created badges are saved to `badge_definitions` with `platform: "digital_curriculum"` and immediately attached to only that module’s `completionBadgeIds[]`. During lesson progress sync, `LessonPlayer.tsx` computes `modulesCompleted` from mapped lesson completion, persists that map to `courseProgress`, and calls callable `awardCourseModuleBadges` for newly completed modules. The callable re-verifies module completion server-side and awards only the configured badge ids for the specific completed module(s) by merging into `users/{uid}.badges.earned`, so learner badge suite tabs reflect module completion rewards without requiring staff actions.

Public certificate sharing now uses branded website links instead of raw Storage URLs. `ensureCertificatePublicShareUrl` creates/updates `certificate_public_links/{shareId}` (readable publicly, owner-writable), and LinkedIn copy actions use `/certificate/:shareId` on the app domain. `PublicCertificatePage` renders recipient + skill + course summary and links to the official PDF asset, giving a cleaner, verifiable public certificate experience.
...
### How the feature works (Digital Curriculum admin web shell + Student/Admin toggle)

Staff with **`Admin`** or **`superAdmin`** roles see a **Student** / **Admin** segmented control in the main web header (`WebNavigation.tsx`). The choice persists in **`localStorage`** (`mortar_admin_view_mode`); when unset, the default is **`admin`** so staff land in admin context. **Admin** view keeps the header minimal (**Mortar** + toggle only on the left; learner nav links are hidden); **Student** view restores full learner navigation and places the toggle with the right-side account controls. While in **Admin** view, staff are routed to **`/admin`** after sign-in / join (unless exempt paths such as login, join, verify-email, onboarding, public certificate); visiting a learner route redirects back to the admin hub. Successful password verification sets nav mode to **Admin** and sends the user to **`/admin`**, which renders the **command center** hub (`AdminCommandCenter.tsx`). Detailed admin tabs live at **`/admin/panel/:tab`** (`Admin.tsx`) and course builders remain under **`/admin/courses/*`** and lesson deck builder under **`/admin/curriculum/.../builder`**, wrapped by **`AdminLayout.tsx`** (sidebar on non-hub routes). If the toggle is **Student** while any of those guarded routes render, **`AdminLayout`** redirects to **`/dashboard`**. **`/admin/auth`** stays a full-screen sibling route outside the shell layout so the password gate is not clipped by hub chrome.
...
**Shop fulfillment (admin):** **`AdminShopOrdersPanel`** on Admin → Shop lists **`shop_orders`** (real-time) with plain-language line items, amounts, and ship-to. Unfulfilled/processing orders appear in the **Admin Command Center** action strip (**Shop orders to fulfill**). Callable **`adminUpdateShopOrderFulfillment`** updates status, tracking, and notes; sends Brevo **`shop_order_fulfillment_update`** when status/tracking changes. Cloud Function **`onShopOrderCreatedAdminPush`** notifies admins (FCM) when a paid order needs fulfillment.

**Payment confirmation emails (Brevo):** After webhook fulfillment — **`payment_shop_order_confirmed`**, **`payment_event_registration_confirmed`**, **`payment_module_purchase_confirmed`**. Templates + subjects: [docs/brevo-transactional-email-templates.md](../docs/brevo-transactional-email-templates.md). Admin → Events: optional **Ticket price ($)** on create (stored as `ticket_price_cents`). Curriculum: **Buy module** + lesson lock (`moduleAccess`, `LessonPlayer` redirects to `/courses/:courseId` instead of error). Checkout success/cancel URLs return to course; `PaymentSuccessPage` auto-navigates to course with `?purchase=success` to refresh `paid_modules`. Course detail hides module/course price badges when the user owns that module (or all paid modules). **How Buy module works:** Course detail uses `module.id` when present, else `curriculumMapping.modules[n].moduleId` (legacy courses saved without `modules[].id`). Checkout calls `createStripeCheckoutSession` with that id; backend `resolveCheckoutPricing` resolves price by course module row or mapping index. Fulfillment grants `membership.paid_modules` with `curriculum_module_id`. Re-save course in admin to persist `modules[].id` on new saves. Test flow: [docs/STRIPE_SETUP.md](../docs/STRIPE_SETUP.md).

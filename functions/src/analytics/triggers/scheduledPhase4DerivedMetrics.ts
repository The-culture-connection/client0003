...
import { writePhase5DerivedMetricsForDay } from "../summary/writePhase5DerivedMetricsForDay";
...
export const scheduledPhase4DerivedMetrics = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      await writePhase5DerivedMetricsForDay(db, yesterdayKey);
      logger.info("scheduledPhase4DerivedMetrics: wrote phase4+phase5 derived for", { yesterdayKey });
    } catch (err) {

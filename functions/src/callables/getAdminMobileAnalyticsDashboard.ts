...
export const getAdminMobileAnalyticsDashboard = onCall(
  ...
  async (request) => {
    ...
    const frictionSnap = await db.collection(ANALYTICS_COLLECTIONS.FRICTION_SUMMARY).limit(200).get();

    const funnels: Record<string, unknown> = {};
    funnelIds.forEach((id, i) => {
      funnels[id] = funnelSnaps[i].exists ? funnelSnaps[i].data() : null;
    });
    ...
    return {
      ...
      daily_metrics_today: todaySnap.exists ? todaySnap.data() : null,
      daily_metrics_yesterday: yesterdaySnap.exists ? yesterdaySnap.data() : null,

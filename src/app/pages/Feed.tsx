import { Heart, MessageCircle, Share2, MoreHorizontal, Calendar, Clock, MapPin, Users, Plus, ArrowRight, BadgeCheck } from 'lucide-react';
                  ...
                  {events.map((event) => (
                    ...
                    >
                      <div className="flex items-center gap-1 mb-1">
                        <Calendar className={`w-3 h-3 ${event.isMortarEvent ? 'text-[#C1121F]' : 'text-accent'}`} />
                        <h3 className="text-xs text-foreground font-medium truncate flex-1">
                          {event.title}
                        </h3>
                        {event.isMortarEvent && (
                          <BadgeCheck className="w-3 h-3 text-[#C1121F] flex-shrink-0" />
                        )}
                      </div>
                      {event.isMortarEvent && (
                        <span className="inline-block bg-[#C1121F]/20 text-[#C1121F] text-[10px] px-1.5 py-0.5 rounded mb-1 border border-[#C1121F]/40">
                          Official Event
                        </span>
                      )}
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl relative overflow-hidden ${
                      item.isMortarEvent
                        ? 'bg-[#C1121F]/5 border-2 border-[#C1121F]/40 shadow-lg shadow-[#C1121F]/10'
                        : 'bg-card border border-border'
                    }`}
                  >
                    {item.isMortarEvent && (
                      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-radial from-[#C1121F]/10 to-transparent rounded-full blur-2xl -z-10" />
                    )}
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        {item.isMortarEvent && (
                          <BadgeCheck className="w-5 h-5 text-[#C1121F] flex-shrink-0" />
                        )}
                      </div>
                      {item.registered && (
                        <span className="bg-accent text-accent-foreground text-xs px-2 py-1 rounded ml-2">
                          Registered
                        </span>
                      )}
                    </div>
                    ...
                    {item.isMortarEvent && (
                      <div className="mb-3 inline-flex items-center gap-1.5 bg-[#C1121F]/20 text-[#C1121F] text-xs px-2.5 py-1 rounded-full border border-[#C1121F]/50">
                        <div className="w-1.5 h-1.5 bg-[#C1121F] rounded-full animate-pulse" />
                        <span className="font-medium uppercase tracking-wide">Official Mortar Event</span>
                      </div>
                    )}

                    <div className="space-y-2 text-sm text-muted-foreground mb-3">
                      <div className="flex items-center gap-2">
                        <Calendar className={`w-4 h-4 ${item.isMortarEvent ? 'text-[#C1121F]' : ''}`} />
                        <span>{item.date}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Clock className={`w-4 h-4 ${item.isMortarEvent ? 'text-[#C1121F]' : ''}`} />
                        <span>{item.time}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <MapPin className={`w-4 h-4 ${item.isMortarEvent ? 'text-[#C1121F]' : ''}`} />
                        <span>{item.location}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Users className={`w-4 h-4 ${item.isMortarEvent ? 'text-[#C1121F]' : ''}`} />
                        <span>
                    ...
                    {item.registered ? (
                      <button className={`w-full py-2 border rounded-lg transition-all ${
                        item.isMortarEvent
                          ? 'border-[#C1121F]/50 text-[#C1121F] hover:bg-[#C1121F]/10'
                          : 'border-border text-foreground hover:bg-secondary'
                      }`}>
                        View Details
                      </button>
                    ) : (
                      <button className={`w-full py-2 rounded-lg transition-all ${
                        item.isMortarEvent
                          ? 'bg-[#C1121F] hover:bg-[#C1121F]/90 text-white shadow-lg shadow-[#C1121F]/30'

...
- **Phase 5 (derived + funnels):** Central query helpers in `functions/src/analytics/queries/dashboardAggregates.ts` compute onboarding / lesson / quiz / engagement rates, churn risk indicators, and funnel conversions over a date window from `daily_metrics` + `user_analytics_summary`. Callable **`getPhase5DashboardMetrics`** exposes this to dashboards (Admin / superAdmin). Notification events are tracked via `notification_item_clicked` (client ingest) and `notification_mark_read_backend` (server callable write).
...
| `analytics_user_badges` | Materialized badge / milestone state per user | Triggers / batch | Owner or staff |
...
Firestore rules deny client reads/writes on raw summaries and badges except as noted in `firestore.rules`.
...
- **Badges**: `badges/badgeRulesEngine.ts` is a no-op placeholder; wire rules that write `analytics_user_badges/{uid}` when ready.

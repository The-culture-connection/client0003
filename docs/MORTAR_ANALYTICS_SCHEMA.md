...
- **Phase 4 (summary layer):** On each new normalized doc in **`analytics_events`** (`schema_version: 2`, including `web_curriculum_vite` and backend writers such as scheduled nudges / notification-read callable), **`onAnalyticsWebEventCreated`** updates **`user_analytics_summary/{userId}`** (per-user counts, `last_active_at`, UTC **streak** fields), **`daily_metrics/{YYYY-MM-DD}`** (UTC day bucket: **DAU** on first touch per user per day, funnel counters, `total_web_events`), **`course_analytics_summary/{courseId}`** when `properties.course_id` is set, and **`community_analytics_summary/global`**. `applyPhase4RollupsForWebEvent` maps many web events to **granular `counts.*` keys** (shop filters, lesson player friction, auth/onboarding funnel volume, etc.) while keeping legacy aggregates where needed (e.g. **`shop_add_to_cart_clicked`** plus **`cart_add_to_cart`** for the same click). **`scheduledPhase4DerivedMetrics`** (01:30 UTC) writes **`derived_*`** fields on yesterday’s `daily_metrics` (quiz pass rate, DAU mirror; placeholders `null` for cart abandonment, lesson latency, course progress % until extended). Raw **`analytics_raw_events`** rollups remain in `onAnalyticsRawEventCreated` / `analytics_daily_summaries` — Phase 4 web summaries are separate collections as specified.
- **Phase 5 (derived + funnels):** Central query helpers in `functions/src/analytics/queries/dashboardAggregates.ts` compute onboarding / lesson / quiz / engagement rates, churn risk indicators, and funnel conversions over a date window from `daily_metrics` + `user_analytics_summary`. Callable **`getPhase5DashboardMetrics`** exposes this to dashboards (Admin / superAdmin) and includes a per-UTC-day **`daily_series`** (same window as totals) for trend-style goal reports. Funnel step definitions align with Phase 4 counters (e.g. first step uses **signups + `login_sign_ins`**, lesson starts include **dashboard continue learning**, curriculum step adds **course card clicks**, community “surface” sums search/category plus **granular** hub preview/start/RSVP counters, shop add-to-cart sums **`shop_add_to_cart_clicked` + legacy `cart_add_to_cart`**, shop visits use **max(filter/size signals, add-to-cart)** so carts without filter tweaks still show a sensible funnel). Optional request field **`include_debug: true`** adds a small **`debug`** object (caller `uid`, `auth_path` token vs server lookup, normalized role arrays from token / Auth custom claims / Firestore `users/{uid}.roles`, UTC window dates, and the aggregated **`totals.counts`** map) for live verification; the Admin Analytics panel currently requests it and shows it in a collapsible block—remove when no longer needed. Notification events are tracked via `notification_item_clicked` (client ingest) and `notification_mark_read_backend` (server callable write).
- **Admin UI surface:** `Digital Curriculum/src/app/components/admin/AnalyticsDashboardPanel.tsx` renders the Phase 5 snapshot by calling `getPhase5DashboardMetrics` and is mounted in `Admin.tsx` under the **Analytics** tab. The same panel includes **Raw Firestore export**: UTC date range + **Download raw JSON**, which pages callable **`queryAdminAnalyticsEventsDateRange`** (streams: `created_at` window, then legacy `timestamp` window; max 92 days per export). **`AdminGoalReportsPanel.tsx`** (Admin → **Reports**) organizes the same snapshot (plus optional **Layer A** rows from callable **`queryAdminWebAnalyticsEvents`**) into goal buckets: overview, conversion timing counters, learning, community, shop, friction, users/segments headline metrics, and admin builder activity.

...
- **Duplicate protection (implemented):** the sync upserts by email, so it can never create two HubSpot contacts with the same address. Two extra controls: (1) if multiple `users` docs resolve to the same email, only the most recently updated one is sent per run (`skippedDuplicateEmail` in the run summary); (2) set `users/{uid}.hubspot_sync_exclude = true` on beta/test accounts to keep them out of the sync entirely (`skippedExcluded`). Note the sync cannot merge/delete contacts already in HubSpot — same-person-different-email records are cleaned up with HubSpot's built-in duplicate management (Contacts → Actions → Manage duplicates). Sync-created records always carry `mortar_uid`; a "duplicate" without it came from an import/form, not the sync.
